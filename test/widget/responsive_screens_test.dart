...
void main() {
  for (final language in ['en', 'mn']) {
    for (final size in [
      ...
      for (final scale in [1.0, 2.0]) {
        testWidgets(
          '$language navigation and writing fit $size with text scale $scale',
          (tester) async {
            ...
            await tester.pumpWidget(
              MongolianHandwritingApp(
                store: onboardedStore(languageCode: language),
              ),
            );
  ...
  testWidgets('settings persist when the app is recreated', (tester) async {
    ...
    await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('Beginner guidance'));
